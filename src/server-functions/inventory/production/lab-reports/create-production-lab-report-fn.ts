...
const createProductionLabReportSchema = z.object({
...
export const createProductionLabReportFn = createServerFn()
    .middleware([requireManufacturingRunManageMiddleware])
    .inputValidator(createProductionLabReportSchema)
    .handler(async ({ data, context }) => {
        const [report] = await db
            .insert(productionRunLabReports)
            .values({
