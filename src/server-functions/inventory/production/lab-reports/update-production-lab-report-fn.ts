...
const updateProductionLabReportSchema = z.object({
...
export const updateProductionLabReportFn = createServerFn()
    .middleware([requireManufacturingRunManageMiddleware])
    .inputValidator(updateProductionLabReportSchema)
    .handler(async ({ data }) => {
        const [report] = await db
            .update(productionRunLabReports)
            .set({
