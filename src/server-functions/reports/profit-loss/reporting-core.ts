...
export async function fetchScopedInvoiceRows(
  scope: Pick<ScopeFilter, "productId" | "recipeId" | "source">,
  range: ReportDateRange,
): Promise<RealizedInvoiceRow[]> {
  const context = await loadContext(range.fromDate, range.toDate, scope.source);
...
export async function fetchScopedSummary(
  ...
  const context = await loadContext(scope.fromDate, scope.toDate, scope.source);
...
export async function fetchScopedTrend(
  ...
  const context = await loadContext(fromDate, toDate, scope.source);
...
export async function fetchProductRecipeBreakdown(
  ...
  const [productRecipes, context] = await Promise.all([
    ...
    loadContext(range.fromDate, range.toDate, source),
  ]);
...
export async function fetchProductRecentInvoices(
  ...
  const rows = await fetchScopedInvoiceRows({ productId, source }, range);
...
export async function fetchRecipeInvoiceDetails(
  ...
  return fetchScopedInvoiceRows({ recipeId, source }, range);
}
