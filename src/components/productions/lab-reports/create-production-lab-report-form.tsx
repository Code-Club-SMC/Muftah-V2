...
import {
    useCreateProductionLabReport,
    useUpdateProductionLabReport,
} from "@/hooks/production/use-production-lab-reports";

type CreateProductionLabReportFormProps = {
    open: boolean;
...
export const CreateProductionLabReportForm = ({
    open,
    onOpenChange,
    productionRunId,
    productName,
    batchId,
    editData,
}: CreateProductionLabReportFormProps) => {
    const createReport = useCreateProductionLabReport();
    const updateReport = useUpdateProductionLabReport();
    ...
    const [stockNumber, setStockNumber] = useState(
        editData?.stockNumber || "",
    );
    const [lotNumber, setLotNumber] = useState(
        editData?.lotNumber || batchId || "",
    );
    const [certifiedBy, setCertifiedBy] = useState(
        editData?.certifiedBy || "",
    );
    const [certifierTitle, setCertifierTitle] = useState(
        editData?.certifierTitle || "",
    );
    ...
    const [standardReference, setStandardReference] = useState(
        editData?.standardReference || "ISO 9001:2015",
    );
    ...
    useEffect(() => {
        if (editData) {
            setProdName(editData.productName);
            setStockNumber(editData.stockNumber || "");
            setLotNumber(editData.lotNumber || batchId || "");
    ...
    const isFormValid = () => {
        if (!prodName.trim() || !certifiedBy.trim() || !reportDate)
            return false;
    ...
    return (
        <ResponsiveSheet
            ...
            description={
                isEditMode
                    ? `Update lab report for ${productName}`
                    : `Create a lab report for ${productName}`
            }
            icon={FlaskConical}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setCertifierTitle(e.target.value)
                                }
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground p-3 bg-muted/30 rounded-lg border">
                    ...
                    <span>
                        ...
                        <span className="text-emerald-600 font-bold">
                            {analysisItems.filter((i) => i.passed).length}{" "}
                            passed
                        </span>{" "}
                        •{" "}
