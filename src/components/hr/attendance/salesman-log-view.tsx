...
import { ArrowLeft, Briefcase, Check, FilterX } from "lucide-react";
...
export const SalesmanLogView = () => {
  const { employeeId } = useParams({
    from: "/_protected/hr/salesman-details/$employeeId",
  });
  const { from, to } = useSearch({
    from: "/_protected/hr/salesman-details/$employeeId",
  });
  const navigate = useNavigate({
    from: "/hr/salesman-details/$employeeId",
  });
  ...
  return (
    <div className="p-8 pt-6 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-start gap-4 mt-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
              Comprehensive delivery performance, credit recovery, and field attendance
            </p>
          </div>
        </div>
      ...
      <Suspense fallback={<GenericLoader title="Loading Salesman Field Log..." />}>
