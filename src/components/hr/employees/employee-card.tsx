...
export function BarcodeSvg({
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(${CODE_128_QUIET_ZONE} 0)`} shapeRendering="crispEdges">
...
function InfoField({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[100px_10px_1fr] items-baseline gap-2 py-2.5">
      ...
      <span className="break-words leading-snug text-[14px] font-medium text-black">{value}</span>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center px-8 text-center">
        ...
        <div className="my-8 flex w-[176px] items-center">
          <div className="h-[3px] flex-1" style={{ background: BRAND.yellow }} />
          <div className="h-[15px] w-[15px] rounded-full" style={{ background: BRAND.yellow }} />
          <div className="h-[3px] flex-1" style={{ background: BRAND.yellow }} />
        </div>
...
export function EmployeeCard({
  ...
  return side === "front" ? (
    <CardFront employee={employee} fullName={fullName} barcodeValue={barcodeValue} />
  ) : (
    <CardBack />
  );
}
