...
import { ChevronLeft, AlertCircle, BookOpen, Settings, CreditCard } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl border bg-card">
          <p className="text-[10px] font-semibold uppercase text-muted-foreground mb-2">Total Profit</p>
          <p className={cn("text-xl font-bold tabular-nums", (customer as any).lifetimeProfit >= 0 ? "text-emerald-700" : "text-red-700")}>
            {PKR(Number((customer as any).lifetimeProfit) || 0)}
          </p>
        </div>
      </div>
          ...
          {editingCredit ? (
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                />
                <label htmlFor="creditHold" className="text-xs font-medium">Hold</label>
              </div>
              <Button
                size="sm"
                onClick={() => {
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm">
                Limit: <span className="font-bold tabular-nums">{PKR(Number(customer.creditLimit) || 0)}</span>
              </span>
              {customer.creditHold && (
                <Badge variant="destructive" className="text-[10px]">Hold</Badge>
              )}
              <Button
                variant="ghost"
                size="icon"
